...
이 저장소는 개인용 유틸리티 웹 앱 Python AI Util(텍스트 추출, PDF 변환, 유튜브 다운로드)이며 `tools.manizu.blog`에서 운영 중입니다. 계획은 `python-AI-유틸리티웹_개발계획서.md`, 작업 체크리스트는 `TASKS.md`, 배포·운영 절차는 `DEPLOYMENT.md`, 수동 기능 점검은 `TESTING_GUIDE.md`에 있습니다. 이름에 AI가 들어가지만 AI 기능은 프로젝트 범위에서 제외되었습니다.
