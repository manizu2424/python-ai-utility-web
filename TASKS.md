...
- [ ] #2 문서 전체의 도메인을 실제 운영 도메인으로 통일
- [ ] #4 Phase 0 체크 상태, 2026-09-04 VPS 배포 기록, 계획서 §7 "다음 액션" 갱신
- [ ] #6 검증 기록 문구를 실제 테스트 범위에 맞게 수정(워커 스레드, 자동 정리, 텍스트 추출, JS 이벤트 확인)
- [ ] #7 Phase 1 현재 상태에 브라우저 수동 회귀 테스트 미수행 여부 표시
- [ ] #8 계획서의 PDF↔Word/Excel 범위를 실제 구현에 맞추고, Excel→PDF 범위 제외 여부와 텍스트 전용 변환 한계 기록
- [ ] #9 `AGENTS.md`의 Git 설명, 구조·문서 목록, AI 관련 지침 갱신
- [ ] #10 TASKS 표현 정리(옛 "준비 중" 기록, 라우터 분리 항목 위치, 우선순위 절, 검증 기록 날짜, 앱 이름 통일)
- [ ] #11 `DEPLOYMENT.md`에 병합 요청 합계 크기 제약과 저장소 clone 인증 절차 추가
- [ ] #12 로그 민감 정보 필터링 항목을 확인 가능한 완료 기준으로 다시 쓰기
- [ ] #13 루트의 서버 스크린샷을 저장소 밖으로 옮기거나 `.gitignore`에 추가
