...
Phase 0~3은 모두 완료했다. 남은 작업은 다음 두 가지이며 위에서부터 진행한다.

1. OCR 전처리 개선과 품질 회귀 테스트 (테스트 작업, "OCR 현황과 개선 메모")
2. VPS 접속 정보와 운영 계정 정리 (Phase 0)
...
- [x] 운영 로그 개인정보 점검 (2026-09-26)
  - uvicorn 접근 로그: 요청 경로·상태 코드·접속 IP만 남고 유튜브 URL, 업로드 파일명, 이미지 프롬프트는 요청 본문이라 남지 않음(운영 모드 로컬 실행으로 확인)
  - NPM 접근 로그: 요청 경로·접속 IP·User-Agent·Referer만 남고 요청 본문과 Basic Auth 사용자명은 남지 않음(운영 서버 로그 확인, "youtube" 검색 결과는 API 경로와 스크립트 파일명뿐)
  - yt-dlp: `quiet`에서도 실패 시 `ERROR: [youtube] <영상 ID>: ...`와 자막 다운로드 진행 표시를 Docker 로그에 출력함 → 출력 없는 로거와 `noprogress` 적용(테스트 2개, 운영 배포 후 Docker 로그의 yt-dlp 출력 0건 확인). 유튜브 실패 원인은 로그에 남지 않으므로 필요하면 로컬에서 재현한다
