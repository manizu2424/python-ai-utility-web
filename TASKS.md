...
- [x] PDF 변환 결과 생성의 이벤트 루프 비차단 처리 검증 (OCR·파일 정리는 코드상 워커 스레드 실행, 테스트는 후속 조치에서 추가)
...
- Tesseract 실행 파일 누락 오류와 만료 파일 정리 함수 검증(주기 실행 루프는 미검증)
...
- PDF 변환 결과 생성의 워커 스레드 실행 검증(OCR·파일 정리는 코드상 적용, 테스트 없음)
...
- [ ] PDF·DOCX 텍스트 추출과 OCR 성공 경로 테스트 추가
- [ ] OCR·파일 정리 작업의 워커 스레드 실행과 주기 정리 루프 테스트 추가
- [ ] `ffmpeg` 누락 시 유튜브 다운로드 오류 테스트 추가
